...
def make_suite(backend_name, Transform_Class):
    class_name = f"Test_{Transform_Class.__name__}_{backend_name.upper()}"

    class TestToTensorTransform(AetherBaseTestCase):
    ...
    TestToTensorTransform.__name__ = class_name
    TestToTensorTransform.__qualname__ = class_name

    return TestToTensorTransform


for backend in backends_to_test:
    class_name = f"Test_{TARGET_CLASS.__name__}_{backend.upper()}"

    globals()[class_name] = make_suite(backend_name=backend, Transform_Class=TARGET_CLASS)