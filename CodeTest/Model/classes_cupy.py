...
class MeanSquaredError:
    ...
    def forward(self, y_pred, y_true):
        sample_loss = cp.mean((y_true - y_pred)**2, axis = -1)
        return sample_loss
    ...
    def update_parameters(self, layer):
        if self.momentum:
            ...
            if not hasattr(layer, 'weight_momentums'):
                layer.weight_momentums = cp.zeros_like(layer.weights)#If there is no momentum array for weigh)ts
                layer.bias_momentums = cp.zeros_like(layer.biases)#The array doesn't exist for biases yet either. 
